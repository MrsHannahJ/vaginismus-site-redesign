<?php
  $pagetitle = 'O vaginismo pode desaparecer sem tratamento?';
  $pagedescription = 'O vaginismo pode desaparecer sem tratamento? O vaginismo é uma disfunção que se auto-perpetua e requer tratamento para quebrar o ‘ciclo da dor’ e superá-la completamente.';
  $pagecanonical = 'https://www.vaginismus.com/por/faq/general-questions/will-vaginismus-go-away/';
  $pageimagepath ="https://www.vaginismus.com/img/twitter-card.png";
  include('../../../../config.php');
  include(ROOT_PATH.'/includes/por/header.php');
?>

    
    <main class="faq-question">
      <div class="wrapper">
<!--   MAIN SECTION   -->
        <section class="large-9">
          <h1>O vaginismo pode desaparecer sem tratamento?</h1>
          <figure>
            <figcaption>
              <h3>O vaginismo é uma disfunção que se auto-perpetua e requer tratamento para quebrar o ‘ciclo da dor’ e superá-la completamente.</h3>
            </figcaption>
            <img src="../../../../img/por/cycle-of-pain-chart-por.png" alt="Ciclo de dor. 1) Anicipate Pain: O corpo antecipa; medo / ansiedade pode contribuir; 2) reação involuntária: O corpo aperta automaticamente músculos vaginais; 3) dolorosa Sexo: Tightens faz sexo doloroso; a penetração pode ser impossível; 4) A dor Reforça: dor se intensifica resposta reflexa; 5) Órtese: corpo reage apoiando mais em uma base contínua; 6) Evitar: Evitar a intimidade, a falta de desejo pode se desenvolver.">
          </figure>
          <p>O vaginismo é uma condição involuntária e autoperpetuadora; por isso, não é normalmente possível resolver espontaneamente, a menos que devidamente tratado. O ciclo da dor que é desencadeado com o vaginismo geralmente continua indefinitivamente (e tipicamente piora) até que a mulher aprenda a controlar o músculo do assoalho pélvico que contrai a abertura vaginal. A mulher não pode, por mais que queira, simplesmente “desejar” que o vaginismo desapareça, e ele não desaparece sozinho.</p>
          <p>Felizmente, o tratamento do vaginismo tem altas <a href="../../../faq/treatment-questions/vaginismus-treatment-success-rates/index.html">taxas de sucesso</a>. Uma recuperação completa do vaginismo é o resultado normal do <a href="../../../vaginismus-treatment/index.html">tratamento</a>, onde ocorre o intercurso sexual sem dor.</p>
          <hr/>
          <h3>Veja Também</h3>
          <ul>
            <li><a href="../../../vaginismus-treatment/index.html">Tratamento do vaginismo</a></li>
            <li><a href="../../../faq/treatment-questions/how-long-does-treatment-take/index.html">Quanto tempo dura o tratamento?</a></li>
            <li><a href="../../../faq/treatment-questions/recommend-vaginismus-treatment-professionals/index.html">Durante o tratamento, é recomendável trabalhar juntamente com profissionais como médicos, ginecologistas, fisioterapeutas, psicólogos, psiquiatras, terapeutas sexuais ou outros?</a></li>
            <li><a href="../../../faq/treatment-questions/vaginismus-diagnosis-and-treatment-difficulties/index.html">Por que é tão difícil obter um diagnóstico e ajuda para tratar o vaginismo?</a></li>
            <li><a href="../../../faq/treatment-questions/can-vaginismus-reoccur/index.html">Uma vez tratado, o vaginismo pode voltar?</a></li>
          </ul>
          <hr class="hidden-desktop"/>
        </section>
        
<!--   SIDEBAR   -->

          <aside class="large-3">
          <h2>Perguntas Gerais</h2>
          <ul>
            <li><a href="../what-is-vaginismus/index.html"><i class="icon-angle-double-right"></i>O que é vaginismo?</a></li>
            <li><a href="../do-i-have-vaginismus/index.html"><i class="icon-angle-double-right"></i>Como saber se tenho vaginismo?</a></li>
            <li><a href="../primary-and-secondary-vaginismus-differences/index.html"><i class="icon-angle-double-right"></i>Qual é a diferença entre vaginismo primário e vaginismo secundário?</a></li>
            <li><a href="../varying-vaginismus-symptoms/index.html"><i class="icon-angle-double-right"></i>Existem diferentes graus dos sintomas do vaginismo?</a></li>
            <li><a href="../will-vaginismus-go-away/index.html" class="active"><i class="icon-angle-double-right"></i>O vaginismo pode desaparecer sem tratamento?</a></li>
            <li><a href="../how-many-women-have-vaginismus/index.html"><i class="icon-angle-double-right"></i>Quantas mulheres têm vaginismo?</a></li>
            <li><a href="../vaginismus-just-in-my-head/index.html"><i class="icon-angle-double-right"></i>O vaginismo pode existir aapenas na minha cabeça?</a></li>
            <li><a href="../vagina-too-small/index.html"><i class="icon-angle-double-right"></i>É possível que a minha vagina seja pequena demais?</a></li>
            <li><a href="../hymen-problems/index.html"><i class="icon-angle-double-right"></i>É possível que o meu hímen seja a causa do problema?</a></li>
            <li><a href="../had-pleasurable-intercourse-before/index.html"><i class="icon-angle-double-right"></i>Eu tinha intercurso sexual prazeroso antes e agora dói. Ainda assim, o meu problema pode ser vaginismo?</a></li>
            <li><a href="../vaginismus-affect-husbands/index.html"><i class="icon-angle-double-right"></i>Como o vaginismo afeta maridos ou namorados de mulheres com vaginismo?</a></li>
            <li><a href="../dyspareunia-diagnosis-treatment/index.html"><i class="icon-angle-double-right"></i>O que é dispareunia? Qual é a conexão entre dispareunia e vaginismo?</a></li>
            <li><a href="../orgasms-problems-from-vaginismus/index.html"><i class="icon-angle-double-right"></i>Eu não consigo ter orgasmos. Isso pode estar relacionado ao vaginismo?</a></li>
          </ul>
          <a href='../../../faq/index.html' class="shaded-btn">Ver Tudo PMFs</a>
        </aside>
      </div>
    </main>
                    
<?php
  include(ROOT_PATH.'/includes/por/footer.php');
?>